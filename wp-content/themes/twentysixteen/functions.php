...
/**
 * Get array of categories that represent particular regions
 *
 * @since Twenty Sixteen 1.0
 *
 * @return array|null an array of categories that represent regions
 */
function get_region_categories() {
    $categoriesArgs = array();
    $regionCategories = array();

    $allCategories = get_categories($categoriesArgs);

    foreach($allCategories as $key => $category){
        if($category->cat_ID === 3 || $category->name === 'Регионы') {
    ...
    if(!empty($parentCategory)) {
        foreach($allCategories as $category){
            if($category->parent === $parentCategory->cat_ID) {
                $regionCategories[] = $category;
            }
        }
    } else {
        //Обработка, когда не найдена подходящая категория
        return null;
    }
